...
function harness() {
  ...
  const invoke = async (command, args) => {
    ...
    if (command === "nas_request") {
      ...
      args.onPacket.onmessage({ type: "chunk", bytes: [...new TextEncoder().encode('data: {"ok":true}\n\n')] });
      args.onPacket.onmessage({ type: "done" });
    }
  };
  const profile = { id: "one", name: "NAS", url: "http://127.0.0.1:8088" };
  return { calls, transport: createNasTransport({ profile, invoke, Channel: class {}, confirm: async () => true, prompt: async () => null }) };
}
test("NAS transport preserves stream bytes and profile identity", async () => {
  const { calls, transport } = harness();
  assert.equal(await (await transport.fetch("/api/ai/chat?stream=true")).text(), 'data: {"ok":true}\n\n');
...
test("disposed NAS transports cannot send requests to another source", async () => {
  const { calls, transport } = harness();
  transport.dispose();
  await assert.rejects(transport.fetch("/api/overview"), { name: "AbortError" });
...
test("aborted requests never enter native transport", async () => {
  ...
  await assert.rejects(transport.fetch("/api/overview", { signal: controller.signal }), { name: "AbortError" });
