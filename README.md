...
系统页还会读取宿主可用的风扇转速传感器，按 CPU 风扇、系统风扇、机箱风扇等名称分别显示 RPM。容器没有暴露 Linux hwmon 传感器时会显示未检测到，不会伪造转速。
...
清理历史统计：进入 **设置 → 历史数据维护 → 清理流量历史**，确认后只删除网卡和进程流量历史。规则、通知渠道、图标、AI 对话及告警证据均保留。对应接口为需要登录的 `POST /api/history/clear`。

不要删除 `traffic.db` 来清理历史：同一数据库也保存配置和告警证据。

容器保护的最大次数指自上次手动重置以来的重启尝试次数，跨应用重启保留。每次重启或停止均须满足条件持续时间；达到上限后再次持续超限将停止容器。停止成功后锁定，停止失败最多尝试 3 次（至少间隔 60 秒），随后锁定并记录原因。可在容器保护规则中使用“重置计数并解除锁定”重新启用监控；该操作本身不会启动已停止的容器。若重启时发现动作结果不确定，也会锁定等待人工确认。

首次从未保存动作计数的旧版本升级时，保护计数从零开始，已有规则继续保留。磁盘保护按不同采样时刻的累计读写字节差计算速率；重复缓存样本不会被当成零速率，过期样本不会触发动作。

Go 采集器内部 API 仅监听 `127.0.0.1`，不需要开放或映射 `18088`。Go 采集数据同时用于实时告警、网卡历史、进程历史和触发时连接证据。历史进程聚合仍包含抓包可见的内网与公网流量；旧版本未保存的进程记录无法补回。
