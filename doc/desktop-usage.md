...
工作流为 `.github/workflows/desktop.yml`。桌面代码在 `main` 或 `codex/desktop-*` 分支变动时自动运行，也可以在 Actions 页面选择 `Desktop preview` → `Run workflow` 手动执行。
...
自动化覆盖首个采样、计数重置、休眠间隔、SQLite 清理隔离、NAS 地址限制、认证失败、cookie 隔离、重定向拒绝、响应上限和请求取消。Mac 还需打开原生应用检查实时数据、历史切换、目标切换、关闭与退出；Windows Actions 通过仅证明 Windows 编译与自动化测试成功，不代表在用户 Windows 硬件上实测。
