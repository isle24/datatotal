...
- 清历史统计请在“设置 → 历史数据维护”中操作，只清除网卡和进程流量历史，保留配置、图标、AI 对话和告警证据。不要删除 `./data/traffic.db`。
...
系统页的“风扇转速”使用宿主暴露的 `psutil.sensors_fans()` / Linux hwmon 数据，所有可读到的风扇都会单独显示，包含 CPU 风扇、系统风扇、机箱风扇和辅助风扇，并以 RPM（转/分）展示。若页面显示“当前环境未暴露风扇转速传感器”，通常是宿主没有加载 hwmon 风扇驱动，或容器运行环境没有暴露对应的 `/sys` 传感器；应用不会用估算值冒充真实转速。
...
例如 `192.168.3.11:1900 -> 239.255.255.250:1900` 是 SSDP 组播发现流量，新版本会归为内网，不会计入公网。已写入 SQLite 的旧历史聚合无法反推源/目标 IP，因此旧误分类数据不会自动重算；需要重新统计时，使用设置页面的历史清理功能，保留数据库内其他配置和证据。
...
- 启动为独立 HTTP 服务（仅监听 127.0.0.1:18088），Python 后端通过内部 HTTP 调用其 API；不对局域网开放
